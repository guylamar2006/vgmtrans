...
ScannerRegistration<WhittakerSnesScanner> s_raresnes("WhittakerSnes", {"spc"});
}

// ; Load DIR address
// 10df: 8f 5d f2  mov   $f2,#$5d
// 10e2: 8f 32 f3  mov   $f3,#$32
BytePattern WhittakerSnesScanner::ptnLoadDIR(
...
// 0ac7: ce        pop   x
BytePattern WhittakerSnesScanner::ptnReadSRCNTable(
...
BytePattern WhittakerSnesScanner::ptnSongLoadDKC(
...
BytePattern WhittakerSnesScanner::ptnSongLoadDKC2(
...
BytePattern WhittakerSnesScanner::ptnVCmdExecDKC(
...
BytePattern WhittakerSnesScanner::ptnVCmdExecDKC2(
...
void WhittakerSnesScanner::searchForWhittakerSnesFromARAM(RawFile *file) {
  ...
  // load sequence
  WhittakerSnesSeq *newSeq = new WhittakerSnesSeq(file, version, addrSeqHeader, name);
  ...
  // scan SRCN table
    WhittakerSnesInstrSet *newInstrSet = new WhittakerSnesInstrSet(
  ...
  // load BRR samples
  SNESSampColl *newSampColl = new SNESSampColl(WhittakerSnesFormat::name, file, spcDirAddr, usedSRCNs);
