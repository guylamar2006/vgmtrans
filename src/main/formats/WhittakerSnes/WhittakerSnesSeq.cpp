...
const uint16_t WhittakerSnesSeq::NOTE_PITCH_TABLE[128] = {
...
WhittakerSnesSeq::WhittakerSnesSeq(RawFile *file, WhittakerSnesVersion ver, uint32_t seqdataOffset, string newName)
    : VGMSeq(WhittakerSnesFormat::name, file, seqdataOffset, 0, newName), version(ver) {
...
WhittakerSnesSeq::~WhittakerSnesSeq(void) {
}

void WhittakerSnesSeq::resetVars(void) {
  VGMSeq::resetVars();
...
bool WhittakerSnesSeq::parseHeader(void) {
  setPPQN(SEQ_PPQN);
...
bool WhittakerSnesSeq::parseTrackPointers(void) {
  for (int i = 0; i < MAX_TRACKS; i++) {
    uint16_t trkOff = readShort(dwOffset + i * 2);
    if (trkOff != 0)
      aTracks.push_back(new WhittakerSnesTrack(this, trkOff));
  }
  return true;
}
...
double WhittakerSnesSeq::getTempoInBPM(uint8_t tempo, uint8_t timerFreq) {
...
WhittakerSnesTrack::WhittakerSnesTrack(WhittakerSnesSeq *parentFile, uint32_t offset, uint32_t length)
...
double WhittakerSnesTrack::getTuningInSemitones(int8_t tuning) {
  return 12.0 * log((1024 + tuning) / 1024.0) / log(2.0);
}

void WhittakerSnesTrack::calculateVolPanFromVolLR(int8_t volL, int8_t volR, uint8_t &midiVol, uint8_t &midiPan) {
...
bool WhittakerSnesTrack::readEvent(void) {
  ...
  if (statusByte >= 0x80) {
    ...
    if (noteByte == 0x80) {
      ...
      spcNotePitch = WhittakerSnesSeq::NOTE_PITCH_TABLE[spcKey];
    ...
    map<uint8_t, WhittakerSnesSeqEventType>::iterator pEventType = parentSeq->EventMap.find(statusByte);
...
void WhittakerSnesTrack::onTickBegin(void) {
}

void WhittakerSnesTrack::onTickEnd(void) {
}

void WhittakerSnesTrack::addVolLR(uint32_t offset,
                             uint32_t length,
                             int8_t spcVolL,
                             int8_t spcVolR,
...
void WhittakerSnesTrack::addVolLRNoItem(int8_t spcVolL, int8_t spcVolR) {
  uint8_t newMidiVol;
  uint8_t newMidiPan;
  calculateVolPanFromVolLR(spcVolL, spcVolR, newMidiVol, newMidiPan);
